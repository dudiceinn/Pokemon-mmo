...
function renderMetatile(refs, tilesPrimary, tilesSecondary, palsPrimary, palsSecondary) {
  ...
  for (let layer = 0; layer < 2; layer++) {
    for (let slot = 0; slot < 4; slot++) {
      ...
      const pal = pals[ref.palette] || pals[0];
      ...
      for (let py = 0; py < BASE_TILE_SIZE; py++) {
        for (let px = 0; px < BASE_TILE_SIZE; px++) {
          ...
          // Index 0 = transparent on top layer
          if (layer === 1 && colorIdx === 0) continue;
          if (layer === 0 && colorIdx === 0) {
            // Bottom layer index 0 — still render as transparent
            continue;
          }
...
async function buildTilesetImage(primaryDir, secondaryDir) {
  ...
  for (let i = 0; i < total; i++) {
    const px = renderMetatile(allMeta[i], tilesPrimary, tilesSecondary, palsPrimary, palsSecondary);
    const col = i % cols;
...
for (const m of MAPS) await extractMap(m);
console.log('\nDone!');
